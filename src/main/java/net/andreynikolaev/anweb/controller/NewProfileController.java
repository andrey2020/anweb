...
public class NewProfileController implements Serializable{
    ...
    public Boolean newProfile(String profileName, String email, String password) {
        if(getProfileService().getProfileById(IdGenerator.getIdbyString(profileName)) != null) return false;

        Profiles newProfile = new Profiles(profileName, email, password);
       
        getProfileService().addProfile(newProfile);
    ...
    public class AdminController implements Serializable {
            ...
            if (profileChanged) {
                ...
                try {
                    FacesContext.getCurrentInstance().getExternalContext().redirect("/" +
                                                                                    getSelectedProfile().getProfileName());
                } catch (IOException ex) {
                    Logger.getLogger(net.andreynikolaev.anweb.controller.AdminController.class.getName()).log(Level.SEVERE,
                                                                                                              null, ex);
                }
            }
        }
        ...
        public Boolean getAdminPage() {
            ...
            if (getSelectedProfile() != null &&
                UtilSession.getLoginName().equals(getSelectedProfile().getProfileName())) {
                result = true;
            } else {
                try {
        ...
        public void saveSelectedProfile() {
            try {
                ...
                e.getValidationResult().getFailures().stream().forEach(( f) -> { FacesContext.getCurrentInstance().addMessage(null,
        ...
        public void exportProfile() {
            fileExport =
                new DefaultStreamedContent(getSelectedProfile().getExportByteArrayInputStream(), "text/xml",
        ...
        public void importProfile(FileUploadEvent event) {
            ...
            try {
                ...
                FacesContext.getCurrentInstance().addMessage(null,
                                                             new FacesMessage(FacesMessage.SEVERITY_ERROR, "EROOR!",
                                                                              e.getMessage()));
            }
        }
